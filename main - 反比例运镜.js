...
function refreshState() {
    let aspox =  - CamP[0] , aspoy =  - CamP[1];
    let rad = Math.sqrt(aspox * aspox + aspoy * aspoy);
    let facing = rad == 0 ? 0 : aspox > 0 ? Math.asin(aspoy / rad) + CamR / 180 * Math.PI : Math.PI - Math.asin(aspoy / rad) + CamR / 180 * Math.PI ;
    Container.position.set(CX + 200 / CamZ * rad * Math.cos(facing) , CY - 200 / CamZ * rad * Math.sin(facing) );
    Container.rotation = CamZ > 0 ? ( - CamR ) / 180 * Math.PI : (180 - CamR ) / 180 * Math.PI;
    Container.scale = 200 / Math.abs(CamZ);
};
...
function renderballs() {
    let Mball = beat > 0 ? (beat > movingball.length ? movingball[Math.floor(beat) - 1] : movingball[Math.floor(beat)] ) : movingball[0];
    let CenX , CenY , lastangle = 180 , angle , radius;
    lastangle = beat > 1 ? (angledata[Math.floor(beat) - 1] == 999 ? (angledata[Math.floor(beat) - 2] == 999 ? find999s(Math.floor(beat) - 1) : angledata[Math.floor(beat) - 2] ) : (angledata[Math.floor(beat) - 1] + 180) ) : (180 + Math.floor(beat) * 180) ;
    radius = beat > 1 ? trackrad[Math.floor(beat) - 1] : trackrad[0];
    if (beat > movingball.length) {
        CenX = ballpos[Math.floor(beat) - 2][0];
        CenY = ballpos[Math.floor(beat) - 2][1]; 
        angle = lastangle + fmol(beat , 1) * -360 * rotatedir[rotatedir.length - 1];
    } else if (beat > 1) {
        CenX = ballpos[Math.floor(beat) - 1][0];
        CenY = ballpos[Math.floor(beat) - 1][1];
        angle = lastangle + fmol(beat , 1) * -1 * rotatedir[Math.floor(beat)] * anglelist[Math.floor(beat)];
    } else {
        CenX = 0;
        CenY = 0;
        angle = lastangle + fmol(beat , 1) * -180;
    };
    let BallX = CenX + radius * Math.cos(angle / 180 * Math.PI);
    let BallY = CenY + radius * Math.sin(angle / 180 * Math.PI);
    balls['ball' + String(3 - Mball)].position.set(BallX , -BallY );
    balls['ball' + String(Mball)].position.set(CenX , -CenY );
...
function drawballtile(Mball) {
    const tile = 'balltile#' + String(Mball);
    balls[tile].clear();
    const points = ballTile['ball#' + String(Mball)];
...
function cameraMove() {
    const target = beat > 1 ? (beat > movingball.length ? [ballpos[Math.floor(beat) - 2][0] + relPlayer[0] , ballpos[Math.floor(beat) - 2][1] + relPlayer[1] ] : [ballpos[Math.floor(beat) - 1][0] + relPlayer[0] , ballpos[Math.floor(beat) - 1][1] + relPlayer[1]]) : relPlayer;
    const Mbpm = beat > 1 ? (beat > movingball.length ? tbpm[tbpm.length - 1] : tbpm[Math.floor(beat) - 1]) : Sbpm;
...
let MoveLastP = [0,0] , lastRelP = [0,0];
function cameraCaculate(event) {
    ...
    if ('relativeTo' in event) {
        const newRel = event['relativeTo'];
        if (CamRel === 'Player' && newRel !== 'Player') {
            lastCamP = CamP.slice();
            relPlayer = [0, 0];
        } else if (CamRel !== 'Player' && newRel === 'Player') {
            lastCamP = CamP.slice();
            relPlayer = [0, 0];
        }
        CamRel = newRel;
    ...
    const rel = hasRelativeTo ? event['relativeTo'] : CamRel;
    ...
    if (hasRelativeTo && rel === 'Player' && CamRel === 'Player') {
        if (dur === 0) {
            lastCamP = CamP.slice();
        }
    }

    if (!'position' in event && 'relativeTo' in event) lastRelP = P.slice();
    MoveLastP = P.slice();
